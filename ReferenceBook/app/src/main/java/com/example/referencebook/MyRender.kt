...
class MyRender(context: Context) : GLSurfaceView.Renderer {
    ...
    override fun onDrawFrame(gl: GL10) {
        ...
        drawPlanet(gl, mercury, mercuryAngle, 0.4f, 1.7f, 1) // Меркурий
        drawPlanet(gl, venus, venusAngle, 0.6f, 1.5f, 2) // Венера
        drawPlanet(gl, earth, earthAngle, 0.8f, 1.3f, 3) // Земля
        drawMoon(gl, moon, earthAngle, moonAngle, 0.8f, 4) // Луна
        drawPlanet(gl, mars, marsAngle, 1.0f, 1.1f, 5) // Марс
        drawPlanet(gl, jupiter, jupiterAngle, 1.5f, 1.0f, 6) // Юпитер
        drawPlanet(gl, saturn, saturnAngle, 1.7f, 0.7f, 7) // Сатурн
        drawPlanet(gl, uranus, uranusAngle, 2.0f, 0.5f, 8) // Уран
        drawPlanet(gl, neptune, neptuneAngle, 2.2f, 0.3f, 9) // Нептун
        drawPlanet(gl, pluto, plutoAngle, 2.5f, 0.2f, 10) // Плутон
    ...
    private fun drawPlanet(gl: GL10, planet: Sphere, angle: Float, radius: Float, rotationSpeed: Float, textureIndex: Int) {
        gl.glPushMatrix()
        gl.glTranslatef(
            cos(Math.toRadians(angle.toDouble())).toFloat() * radius,
            sin(Math.toRadians(angle.toDouble())).toFloat() * radius,
            -5.2f
        )
    ...
    private fun drawMoon(gl: GL10, moon: Sphere, earthAngle: Float, moonAngle: Float, radius: Float, textureIndex: Int) {
        gl.glPushMatrix()
        ...
        gl.glTranslatef(earthX, earthY, -5.2f)

        val moonDistance = 0.13f
